...
def ALEX_NET(_X, weights, biases, keep_prob):
    # Reshape input picture
    _X = tf.reshape(_X, shape=[-1, 28, 28, 1])

    # Convolution Layer
    CONV_01     = CONVOLUTION('CONV_01', _X, weights['WEI_CONV_01'], biases['BIA_CONV_01'])
    ...
    Pred_m   = tf.matmul(DENSE_02, weights['out']) + biases['out']
    return Pred_m
...
while step * batch_size < training_iters:
    ...
    sess.run(optimizer, feed_dict={X_Input: batch_xs, Y_Output: batch_ys, keep_prob: keep_prob})
    if step % display_step == 0:
        # Calculate batch accuracy
        acc = sess.run(accuracy, feed_dict={X_Input: batch_xs, Y_Output: batch_ys, keep_prob: 1.})
        # Calculate batch loss
        loss = sess.run(cost, feed_dict={X_Input: batch_xs, Y_Output: batch_ys, keep_prob: 1.})
        print("Iter " + str(step*batch_size) + ", Minibatch Loss= " + "{:.6f}".format(loss) + ", Training Accuracy= " + "{:.5f}".format(acc))
    step += 1
print("Optimization Finished!")
# Calculate accuracy for 256 mnist test images
print("Testing Accuracy:", sess.run(accuracy, feed_dict={X_Input: mnist.test.images[:256], Y_Output: mnist.test.labels[:256], keep_prob: 1.}))
"""
