...
class Model:
        ...
        with tf.variable_scope(self.name):
            ...
            for i in range(3):
                ...
                if i == 2: # for last layer - add 1x1 convolution
                    net = conv_bn_activ_dropout(name="1x1conv", x=net, n_filters=n_filters, kernel_size=[1,1], 
                                                strides=1,dropout_rate=dropout_rate, training=self.training, seed=self.seed)
    ...
    #             net = tf.layers.dropout(net, rate=0.5, training=self.training)
            self.logits = tf.layers.dense(net, 10, kernel_initializer=tf.contrib.layers.xavier_initializer(seed=self.seed),
                                          name="logits")
...
print('Accuracy:', m1.get_accuracy(mnist.test.images[:512], mnist.test.labels[:512],training=False))
...
def evaluate(X_sample, y_sample, batch_size=512):
    ...
    for i in range(0, N, batch_size):
        ...
        correct_sample += sess.run(m1.get_accuracy, feed_dict=feed) * N_batch
