        ...
        with tf.variable_scope(self.name):
            ...
            with tf.variable_scope("conv1"):
                for i in range(layer_n):
                    net = self.wide_residual_block(net, 64, dropout_rate=dropout_rate, 
                                                   first_block=(i==0), name="resblock{}".format(i+1))
                    assert net.shape[1:] == [28, 28, 64]

            with tf.variable_scope("conv2"):
                for i in range(layer_n):
                    net = self.wide_residual_block(net, 128, dropout_rate=dropout_rate, 
                                                   downsampling=(i==0), name="resblock{}".format(i+1))
                    assert net.shape[1:] == [14, 14, 128]

            with tf.variable_scope("conv3"):
                for i in range(layer_n):
                    net = self.wide_residual_block(net, 256, dropout_rate=dropout_rate, 
                                                   downsampling=(i==0), name="resblock{}".format(i+1))
